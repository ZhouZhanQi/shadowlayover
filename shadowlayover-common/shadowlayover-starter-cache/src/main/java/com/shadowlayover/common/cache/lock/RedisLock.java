package com.shadowlayover.common.cache.lock;
...
public class RedisLock {
        ...
        if (locked) {
            return redisTemplate.execute((RedisCallback<Boolean>) connection -> {
                ...
                if (nativeConnection instanceof LettuceClusterConnection) {
                    result = ((LettuceClusterConnection) nativeConnection).scriptingCommands()
                            .eval(UNLOCK_LUA.getBytes(StandardCharsets.UTF_8), ReturnType.INTEGER, 1, lockKey.getBytes(StandardCharsets.UTF_8), lockValue.getBytes(StandardCharsets.UTF_8));
                }

                // 单机模式
                if (nativeConnection instanceof LettuceConnection) {
                    result = ((LettuceConnection) nativeConnection).scriptingCommands()
                            .eval(UNLOCK_LUA.getBytes(StandardCharsets.UTF_8), ReturnType.INTEGER, 1, lockKey.getBytes(StandardCharsets.UTF_8), lockValue.getBytes(StandardCharsets.UTF_8));
                }
    ...
    private String set(final String key, final String value, final long seconds) {
        ...
        return redisTemplate.execute((RedisCallback<Boolean>) connection -> {
            ...
            if (nativeConnection instanceof LettuceConnection) {
                result = ((LettuceConnection) nativeConnection).stringCommands()
                        .set(key.getBytes(StandardCharsets.UTF_8), value.getBytes(StandardCharsets.UTF_8),  Expiration.from(seconds, TimeUnit.SECONDS), RedisStringCommands.SetOption.SET_IF_ABSENT);
            }
